...
{
  mrb_int subkey_len, subkey_id;
  const char *ctx;
  mrb_value key;
  mrb_get_args(mrb, "iizS", &subkey_len, &subkey_id, &ctx, &key);
...
mrb_hydro_kdf_gem_init(mrb_state *mrb, struct RClass *hydro_mod)
{
  ...
  mrb_define_module_function(mrb, hydro_kdf_mod, "derive_from_key", mrb_hydro_kdf_derive_from_key, MRB_ARGS_REQ(4));
}
